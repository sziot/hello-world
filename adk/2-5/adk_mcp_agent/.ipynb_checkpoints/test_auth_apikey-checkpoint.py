...
auth_scheme, auth_credential = token_to_scheme_credential(
    token_type="apikey",           # token_type: API Key 类型
    location="header",            # location: 查询参数位置
    name="X-API-Key",              # name: 参数名
    credential_value=amap_api_key                 # credential_value: API Key 值
)

print(f"ADK 认证配置:")
print(f"  API Key: {amap_api_key}")
print(f"  Auth Scheme: {auth_scheme}")
print(f"  Auth Credential: {auth_credential}")
...
amap_mcp_server = MCPToolset(
    ...
    auth_scheme=auth_scheme,  # 添加认证配置
    auth_credential=auth_credential,  # 添加认证凭据
    tool_filter=[
        # 地址与坐标转换
        "maps_geo",           # 地址转坐标
...
root_agent = LlmAgent(
    ...
    - **geocode**：当用户提供详细地址时，调用该工具获取经纬度坐标。
...
async def chat(session_service: InMemorySessionService, query: str):
    ...
    try:
        ...
        await amap_mcp_server.close()   # 先关闭MCP子进程
        await runner.close()
